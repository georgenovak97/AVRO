...
- Placing a family from an older Revit version retries once in a fresh
  transaction after the one-time format update, so the first double-click can
  continue to placement without a second attempt.
